...
export function createUseScopedState<T, Name extends string>(
    ...
    Record<
        ...
        (props: { children: ReactNode; initialState?: T | (() => T) }) => JSX.Element
    >;
export function createUseScopedState<T, Name extends string>(
    name: Name
): Record<`use${Capitalize<Name>}`, () => UseNamedStateReturnType<T, Name>> &
...
export function createUseScopedState<T, Name extends string>(
    ...
    const useXyz = function () {
        ...
        if (out === undefined) {
            throw new Error(`Must be used in a component wrapped in <${capitalize(name)}Provider />`);
        }
    ...
    const XyzProvider = function (props: { children: ReactNode; initialState?: T | (() => T) }) {
        const { children, initialState: initialStateFromProps } = props;

        const useNamedStateReturnedWrapper = useNamedState(
            name,
            (initialStateFromProps !== undefined ? initialStateFromProps : initialState)!
        );

        const value = useMemo(() => useNamedStateReturnedWrapper, [useNamedStateReturnedWrapper[name]]);
